...
/*
 * Report Slug Page: shows a detailed analysis report by id, and allow update (feature not implemented)
 */
export default function ViewReportSlug() {
  const router = useRouter();
  const { id } = useParams();

  const report = useAppSelector(
    (state: RootState) =>
      state.report.data.data.find((r) => r.id === Number(id)) as Report
  );
